...
import { Layers, Download, FolderPlus, FolderOpen, ChevronLeft, Upload, Eye, Pencil, Trash2, Share2, Copy } from 'lucide-react';

export interface Model3dFolder {
  id: string;
  name: string;
  count: number;
  date: string;
...
const INITIAL_FOLDERS: Model3dFolder[] = [
  { id: 'mf1', name: 'XX区三维_20240520', count: 1, date: '2024-05-20' },
  { id: 'mf2', name: '工业园区实景_20240615', count: 1, date: '2024-06-15' },
];

const INITIAL_ITEMS: Model3dItem[] = [
  { id: 'm1', folderId: 'mf1', name: 'XX区三维实景模型_20240520', status: '成功', time: '2024-05-20', size: '1.2 GB' },
  { id: 'm2', folderId: 'mf2', name: '工业园区倾斜摄影模型_20240615', status: '成功', time: '2024-06-15', size: '2.5 GB' },
];

export default function Model3dResult() {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const folderInputRef = useRef<HTMLInputElement>(null);
  const [viewMode, setViewMode] = useState<ViewMode>('list');
  const [folders, setFolders] = useState<Model3dFolder[]>(INITIAL_FOLDERS);
  const [items, setItems] = useState<Model3dItem[]>(INITIAL_ITEMS);
  ...
  const [previewItem, setPreviewItem] = useState<Model3dItem | null>(null);
  const [editItem, setEditItem] = useState<Model3dItem | null>(null);
  const [editForm, setEditForm] = useState({ name: '', status: '', time: '', size: '' });

  type ShareExpiry = '1d' | '7d' | '1m' | '3m' | 'permanent' | 'custom';
  const [shareItem, setShareItem] = useState<Model3dItem | null>(null);
  ...
  const openShare = (item: Model3dItem) => {
    setShareItem(item);
    setShareLink(null);
  ...
  const generateShareLink = () => {
    ...
    const params = new URLSearchParams({ type: 'model3d', id: shareItem.id, name: shareItem.name });
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filterName.trim()) list = list.filter((i) => i.name.toLowerCase().includes(filterName.trim().toLowerCase()));
  ...
  const handleDownloadOne = (item: Model3dItem) => {
    if (item.file) {
  ...
  const handleDeleteOne = (item: Model3dItem, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const openEdit = (item: Model3dItem, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditItem(item);
  ...
  const breadcrumbs = [
    ...
    { label: '三维模型', path: currentFolderId ? '/data/model3d' : undefined },
                      ...
                      <tr key={item.id} className="border-t border-border hover:bg-muted/20">
                        ...
                        <td className="px-3 py-2" onClick={(e) => e.stopPropagation()}>
                          <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => openShare(item)} className="text-primary hover:underline text-xs flex items-center gap-1" title="分享">
                              <Share2 size={12} /> 分享
                            </button>
                  ...
                  <div key={item.id} className="relative rounded-lg border border-border bg-card/50 overflow-hidden hover:border-primary/50 transition-colors">
                    ...
                    <div className="p-3 border-t border-border">
                      ...
                      <div className="flex gap-2 mt-2 flex-wrap">
                        ...
                        <button type="button" onClick={() => openShare(item)} className="text-primary hover:underline text-xs flex items-center gap-1"><Share2 size={12} /> 分享</button>
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          {previewItem && (
            <div className="rounded-lg border border-border bg-muted/30 p-6 flex flex-col items-center justify-center min-h-[200px]">
              ...
              <p className="text-sm text-muted-foreground">三维模型预览（可接入专业 viewer）</p>
