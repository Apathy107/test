  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-card border border-border p-3 rounded text-xs" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
        ...
        {payload.map((e, i) => <p key={i} style={{ color: e.color }}>{e.name}: {e.value}{typeof e.value === 'number' ? (e.name.includes('率') || e.value <= 100 ? '%' : '') : ''}</p>)}
      </div>
    );
  }
  return null;
          ...
          <TechCard title="异常类型 TOP3" icon={AlertCircle} className="h-full">
            <div className="space-y-3 pt-2">
              ...
              <div className="text-center pt-1">
                <button type="button" className="text-xs text-primary hover:underline">查看完整异常日志 →</button>
              </div>
            </div>
          </TechCard>
        </div>
